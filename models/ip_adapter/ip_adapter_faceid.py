...
class IPAdapterFaceID:
    ...
    def load_ip_adapter(self):
        ...
        if os.path.splitext(self.ip_ckpt)[-1] == ".safetensors":
            model_dict = {}
            with safe_open(self.ip_ckpt, framework="pt") as f:
                for key in f.keys():
                    model_dict[key] = f.get_tensor(key)
                    if key.startswith("image_proj_model"):
                        # image_proj_model.state_dict()[key.replace('image_proj_model.', '')].copy_(f.get_tensor(key))
                        self.image_proj_model.state_dict()[key.replace('image_proj_model.', '')].copy_(f.get_tensor(key))
        else:
            state_dict = torch.load(self.ip_ckpt, map_location="cpu")
        
        for attn_processor  in unet.attn_processors:
            for key in unet.attn_processors[attn_processor].state_dict():
                unet.attn_processors[attn_processor].state_dict()[key] = model_dict['unet.'+attn_processor + '.'+ key]
...
class IPAdapterFaceIDPlusWithControlNet:
